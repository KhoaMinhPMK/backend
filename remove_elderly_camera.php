...
try {
    ...
    if (!$existing) {
        echo json_encode(['success' => false, 'message' => 'No cameras found for this user']);
        exit;
    }
    ...
    if (!in_array($cameraUrl, $currentLinks)) {
        echo json_encode(['success' => false, 'message' => 'Camera URL not found for this user']);
        exit;
    }
    
    // Remove the URL from the array
    $updatedLinks = array_values(array_filter($currentLinks, function($link) use ($cameraUrl) {
        return $link !== $cameraUrl;
    }));
    
    if (empty($updatedLinks)) {
        // If no cameras left, delete the entire record
        $deleteStmt = $pdo->prepare("DELETE FROM users_with_cameras WHERE private_key = ?");
        $deleteStmt->execute([$elderlyPrivateKey]);
        
        echo json_encode([
            'success' => true,
            'message' => 'Camera removed successfully. No cameras remaining.',
            'data' => [
        ...
        // Update the record with remaining cameras
        $updatedLinksJson = json_encode($updatedLinks);
        $updateStmt = $pdo->prepare("UPDATE users_with_cameras SET camera_links = ? WHERE private_key = ?");
        $updateStmt->execute([$updatedLinksJson, $elderlyPrivateKey]);
        ...
        echo json_encode([
            ...
            'data' => [
                'elderly_name' => $elderly['userName'],
                'camera_url' => $cameraUrl,
                'total_cameras' => count($updatedLinks),
                'remaining_cameras' => $updatedLinks
            ]
        ]);
    }
