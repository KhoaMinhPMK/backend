...
error_log("=== REJECT_FRIEND_REQUEST.PHP CALLED ===");
...
try {
    ...
    // 1. Update friend request status to 'rejected'
    $updateRequestSql = "UPDATE friend_requests SET status = 'rejected', responded_at = NOW() WHERE id = ?";
    $stmt = $conn->prepare($updateRequestSql);
    $stmt->execute([$requestId]);
    
    error_log("✅ Friend request marked as rejected");
    
    // 2. Create notification for the requester (fromPhone)
    $notification_data = [
        'type' => 'friend_request_rejected',
        'title' => 'Lời mời kết bạn bị từ chối',
        'body' => $toUser['userName'] . ' đã từ chối lời mời kết bạn của bạn.',
        'details' => [
            'rejecter_phone' => $toPhone,
            'rejecter_name' => $toUser['userName'],
    ...
    // 3. Get full notification data to send via socket
    ...
    // 4. Send real-time notification to the requester
    ...
    error_log("✅ Reject friend request completed successfully");
    sendSuccessResponse($responseData, 'Friend request rejected successfully');
